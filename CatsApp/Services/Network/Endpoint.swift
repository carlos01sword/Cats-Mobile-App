...
extension Endpoint {
    func makeRequest(baseURL: URL, defaultHeaders: [String: String] = [:]) -> URLRequest? {
        var cleanPath = path
        ...
        (defaultHeaders.merging(headers) { _, new in new }).forEach { request.setValue($1, forHTTPHeaderField: $0) }
        return request
    }
}
